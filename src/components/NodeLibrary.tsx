...
export function NodeLibrary(){
  ...
  return <div className="library-backdrop" onMouseDown={()=>setLibraryOpen(false)}><section className="node-library" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="library-results">{categories.map(([id,label])=>{const items=filtered.filter(o=>o.category===id);return items.length?<div className="library-category" key={id}><h3>{label}<span>{items.length}</span></h3><div>{items.map(operator=><button key={operator.id} onClick={()=>addOperator(operator.id)}><i className={`family-${operator.family.toLowerCase()}`}>{operator.family}</i><span><strong>{operator.label}</strong><small>{operator.description}</small></span><em>＋</em></button>)}</div></div>:null})}</div>
  </section></div>
}
